...
def categorical_to_dummy(data, heads):
    for head in heads:
        if (data[head].dtype.name == 'object' ):
            data[head] = pd.Categorical(data[head])
            data[head] = data[head].cat.codes
    return data
...
combined = categorical_to_dummy(combined, combined.head())
